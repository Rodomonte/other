...
def bet_favor(mode, salt, chars):
  ...
  if mode == 'm':
    ...
    elif abs(rwr-bwr) < 0.2: bet = salt * 0.03
    elif abs(rwr-bwr) < 0.3: bet = salt * 0.08
    elif abs(rwr-bwr) < 0.4: bet = salt * 0.15
    elif abs(rwr-bwr) < 0.5: bet = salt * 0.25
    else:                    bet = salt * 0.4
  elif mode == 't':
...
def bet_upset(mode, salt, chars):
  ...
  if mode == 'm':
    ...
    rwr += chars[0][1].wr if len(chars[0]) == 2 else 0.0
    bwr += chars[1][1].wr if len(chars[1]) == 2 else 0.0
  col = 'r' if (abs(rwr-bwr) < 0.2 and rwr < bwr) or \
            (abs(rwr-bwr) >= 0.2 and rwr > bwr) else 'b'
  return (col, bet)
